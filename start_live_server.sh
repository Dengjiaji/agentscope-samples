...
#    - 之后每天 22:30:15：运行盘前分析（func1）生成交易信号
#    - 每天凌晨 05:05：执行交易（func2）并更新前一天的 agent 表现
...
echo "   2. 第一天启动：立即运行盘前分析（生成信号）"
echo "   3. 交易循环："
echo "      • 每天 22:30:15：运行盘前分析（func1）"
echo "      • 每天 05:05：执行交易（func2）+ 更新前一天 agent 表现"
if [ "$MOCK_MODE" = false ]; then
    echo "   4. 实时价格每10秒更新一次（Finnhub Quote API）"
else
    echo "   4. Mock模式下每5秒生成虚拟价格更新"
fi
