...
class PollingPriceManager:
    ...
    def __init__(self, api_key: str, poll_interval: int = 10):
        ...
        self.open_prices: Dict[str, float] = {}  # Store open prices (for calculating return)
        self.price_callbacks: List[Callable] = []
        
        self.running = False
        self.thread = None
        
        logger.info(f"✅ PollingPriceManager initialized (polling interval: {poll_interval}s)")
            ...
            try:
                ...
                if current_price and current_price > 0:
                    ...
                    price_data = {
                        ...
                        'change': quote_data.get('d'),  # Change relative to previous close ($)
                        'change_percent': quote_data.get('dp')  # Change relative to previous close (%)
                    }
                    ...
                    if old_price:
                        ...
                        logger.info(f"💹 {symbol}: ${current_price:.2f} ({change:+.2f}% from last) [Open ret: {ret:+.2f}%]")
                    else:
                        logger.info(f"💹 {symbol}: ${current_price:.2f} (initial) [Open ret: {ret:+.2f}%]")
                else:
                    logger.warning(f"⚠️ {symbol}: Invalid price data (c={current_price})")
    ...
    def start(self):
        ...
        logger.info(f"✅ Price polling manager started (subscribed: {', '.join(self.subscribed_symbols)})")
