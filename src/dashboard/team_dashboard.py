...
class TeamDashboardGenerator:
    ...
    def __init__(self, dashboard_dir: Path, initial_cash: float = 100000.0, 
        ...
        Args:
            ...
            price_data_dir: Price data directory path (default: src/data/ret_data)
        """
    ...
    def update_from_day_result(self, date: str, pre_market_result: Dict[str, Any], 
        ...
        if has_closing_prices:
            ...
            print(f"⏸️  Skipping equity curve update ({date}) - waiting for closing price data")
        
        # 8. Save internal state
        state['last_update_date'] = date
        state['curves_updated'] = curves_updated  # Record whether curves were updated
        ...
        if curves_updated:
            ...
            print(f"⏸️  {date} Skipping equity curve update - waiting for closing price data")
    ...
    def _initialize_buy_and_hold(self, date: str, available_tickers: list, state: Dict):
        ...
        print(f"✅ Buy & Hold strategy initialized: {len(initial_allocation)} stocks, invested ${total_invested:,.2f}")
        for ticker, info in initial_allocation.items():
            print(f"   {ticker}: {info['qty']} shares @ ${info['buy_price']:.2f}")
    ...
    def _calculate_buy_and_hold_value(self, date: str, state: Dict) -> float:
        ...
        if not baseline_state['initialized']:
            return self.initial_cash  # Not initialized yet, return initial cash
        ...
        for ticker, info in initial_allocation.items():
            ...
            if current_price is None or current_price <= 0:
                ...
                print(f"⚠️ Unable to get price for {ticker} on {date}, using purchase price ${current_price:.2f}")
    ...
    def _update_baseline_curve(self, date: str, timestamp_ms: int, state: Dict):
        ...
        if baseline_state['initialized'] and len(state['baseline_history']) == 0:
            ...
            print(f"📊 Buy & Hold initial point: ${self.initial_cash:,.2f} at {date} 05:00:00")
        ...
        print(f"📊 Buy & Hold baseline: ${baseline_value:,.2f} ({return_pct:+.2f}%)")
    ...
    def _initialize_buy_and_hold_vw(self, date: str, available_tickers: list, state: Dict):
        ...
        if not available_tickers:
            print("⚠️ No tradable stocks, skipping value-weighted Buy & Hold initialization")
            return
        ...
        if not market_caps:
            print("⚠️ Unable to get market cap data for any stock, skipping value-weighted Buy & Hold initialization")
            return
        ...
        print(f"✅ Value-weighted Buy & Hold strategy initialized: {len(initial_allocation)} stocks, invested ${total_invested:,.2f}")
        for ticker, info in initial_allocation.items():
            print(f"   {ticker}: {info['qty']} shares @ ${info['buy_price']:.2f} (weight: {info['weight']*100:.2f}%)")
    ...
    def _update_baseline_vw_curve(self, date: str, timestamp_ms: int, state: Dict):
        ...
        if baseline_vw_state['initialized'] and len(state['baseline_vw_history']) == 0:
            ...
            print(f"📊 Value-weighted Buy & Hold initial point: ${self.initial_cash:,.2f} at {date} 05:00:00")
        ...
        print(f"📊 Value-weighted Buy & Hold baseline: ${baseline_vw_value:,.2f} ({return_pct:+.2f}%)")
    ...
    def _calculate_momentum_scores(self, date: str, available_tickers: list, 
        ...
        for ticker in available_tickers:
            ...
            past_price = None
            for days_back in range(lookback_days, lookback_days + 10):  # Search up to 10 more days
    ...
    def _rebalance_momentum_portfolio(self, date: str, available_tickers: list, state: Dict):
        ...
        if not momentum_scores:
            print(f"⚠️ {date} Unable to calculate momentum scores, skipping rebalance")
            return
        ...
        if top_tickers:
            ...
            for ticker in top_tickers:
                ...
                if quantity > 0:
                    ...
                    total_invested += cost
                    print(f"   Buy {ticker}: {quantity} shares @ ${buy_price:.2f}")
        
        # Update state
        momentum_state['initialized'] = True
        momentum_state['last_rebalance_date'] = date
        
        print(f"✅ Momentum strategy rebalance completed, remaining cash: ${momentum_state['cash']:,.2f}")
    ...
    def _update_momentum_curve(self, date: str, timestamp_ms: int, 
        ...
        if momentum_state['initialized'] and len(state['momentum_history']) == 0:
            ...
            print(f"📊 Momentum strategy initial point: ${self.initial_cash:,.2f} at {date} 05:00:00")
        ...
        print(f"📊 Momentum strategy: ${momentum_value:,.2f} ({return_pct:+.2f}%)")
    ...
    def _generate_summary(self, state: Dict):
        ...
        summary = {
            ...
            'baseline_vw': state.get('baseline_vw_history', []),  # Value-weighted baseline
            'momentum': state.get('momentum_history', [])  # Add momentum strategy data
        }
    ...
    def _generate_holdings(self, state: Dict):
        ...
        for ticker, pos in positions.items():
            ...
            print(f"   {ticker}: qty={qty}, current_price=${current_price:.2f}, market_value=${market_value:.2f}, weight={weight:.2%}")
    ...
    def _generate_trades(self, state: Dict):
        ...
        for trade in sorted_trades[:100]:
            ...
            formatted_trade = {
                'id': trade.get('id'),
                'timestamp': trade.get('ts'),  # Keep millisecond timestamp, frontend will format
                'side': trade.get('side'),
    ...
    def _generate_leaderboard(self, state: Dict):
        ...
        ranking_entries.sort(key=lambda x: (
            ...
            -(x['winRate'] if x['winRate'] is not None else 0),  # Win rate descending
        ))
