...
def _display_directory_info(base_data_dir: Path) -> None:
    """Display directory size and last modified time."""
    # 提取显示目录大小的逻辑
    try:
        total_size = sum(
...
def _backup_config_files(base_data_dir: Path, config_name: str) -> None:
    ...
    for backup_file in backup_files:
        ...
        if file_path.exists():
            if backup_dir is None:
                ...
                backup_dir = (
                    base_data_dir.parent / f"{config_name}_backup_{timestamp}"
                )
...
def handle_history_cleanup(config_name: str, auto_clean: bool = False) -> None:
    ...
    # Show directory info (减少1个分支)
    _display_directory_info(base_data_dir)
    console.print()
    ...
    if should_clean:
        ...
        # Backup config files (减少多个分支)
        _backup_config_files(base_data_dir, config_name)
...
def _build_live_command(
    ...
    port: int,
    mock: bool,
    pause_before_trade: bool,
    time_accelerator: float,
    virtual_start_time: Optional[str],
) -> list[str]:
    """构建命令行参数"""
    cmd = [
        sys.executable,
...
def live(
    ...
    _display_configuration(mock, config_name, host, port, pause_before_trade)
    _display_functionality(mock, virtual_start_time, port)
