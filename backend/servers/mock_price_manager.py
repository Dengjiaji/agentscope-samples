...
class MockPriceManager:
    ...
    def subscribe(self, symbols: List[str], base_prices: Dict[str, float] = None):
        ...
        for symbol in symbols:
            if symbol not in self.subscribed_symbols:
                ...
                self.open_prices[symbol] = base_price  # Open price equals base price
                self.latest_prices[symbol] = base_price
                
                logger.info(f"✅ Subscribed to Mock price: {symbol} (Base price: ${base_price:.2f})")
    ...
    def _generate_price_update(self, symbol: str) -> float:
        ...
        if random.random() < 0.1:
            ...
            if abs(trend_factor) > 1:
                logger.debug(f"📰 {symbol} large fluctuation: {trend_factor:+.2f}%")
            ...
            try:
                ...
                for callback in self.price_callbacks:
                    try:
                        ...
                        logger.error(f"Mock price callback error ({symbol}): {e}")
                
                # Log price change
                change = ((new_price - old_price) / old_price) * 100
                logger.debug(f"💹 Mock {symbol}: ${new_price:.2f} ({change:+.2f}%) [Open: {change_from_open:+.2f}%]")
    ...
    def _polling_loop(self):
        ...
        logger.info(f"🚀 Mock price generation started (interval: {self.poll_interval}s, volatility: {self.volatility}%)")
    ...
    def start(self):
        ...
        logger.info(f"✅ Mock price manager started (subscribed: {', '.join(self.subscribed_symbols)})")
    ...
    def reset_open_prices(self):
        ...
        for symbol in self.subscribed_symbols:
            ...
            logger.debug(f"📊 {symbol} New open price: ${new_open:.2f} (Gap: {gap_percent:+.2f}%)")
