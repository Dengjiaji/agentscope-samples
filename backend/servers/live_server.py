...
class LiveTradingServer:
    ...
    def __init__(self, config: LiveThinkingFundConfig, mock_mode: bool = False, pause_before_trade: bool = False, time_accelerator: float = 1.0, virtual_start_time: Optional[datetime] = None):
        ...
        if mock_mode and time_accelerator != 1.0:
            ...
            logger.info(f"🕐 Virtual clock enabled: acceleration {time_accelerator}x, start time: {virtual_start_time or 'current time'}")
        else:
        ...
        if mock_mode:
            ...
            if not api_key:
                logger.error("❌ FINNHUB_API_KEY not found, cannot use real-time price feature")
            ...
            logger.info("📊 Using Finnhub real-time prices (high-frequency polling: 10 seconds)")
    ...
    def _load_internal_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.warning(f"⚠️ Unable to read internal state file, using default values: {e}")
            return default_state
                    ...
                    try:
                        ...
                        if msg_type == 'ping':
                            ...
                            if not self.mock_mode:
                                ...
                                minutes = data.get('minutes', 30)  # Default fast forward 30 minutes
                                try:
                                    old_time = self.vclock.now()
                                    self.vclock.fast_forward(minutes)
                                    new_time = self.vclock.now()
                                    
                                    logger.info(f"⏩ Time fast forwarded: {minutes} minutes ({old_time.strftime('%H:%M:%S')} → {new_time.strftime('%H:%M:%S')})")
    ...
    async def run_live_trading_simulation(self):
        ...
        if self.mock_mode:
            ...
            logger.info(f"🎭 Mock mode: Subscribed to {len(self.config.tickers)} stocks, using virtual prices")
        else:
            self.price_manager.subscribe(self.config.tickers)
            logger.info(f"📊 Real-time mode: Subscribed to {len(self.config.tickers)} stocks, using Finnhub API")
        
        self.price_manager.start()
        logger.info(f"✅ Price manager started, real-time update frequency: {'Every 5 seconds (Mock)' if self.mock_mode else 'Every 10 seconds (Finnhub)'}")
        ...
        logger.info(f"📅 Current Beijing time: {reference_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        logger.info("===== [Continuous Monitoring] Entering continuous operation mode =====")
    ...
    async def _run_pre_market_analysis(self, date: str):
        ...
        if not isinstance(result, dict):
            logger.error(f"❌ Analysis returned wrong type: expected dict, got {type(result).__name__}")
        ...
        if result.get('status') != 'success':
            logger.warning(f"⚠️ Pre-market analysis not successful: {result.get('reason', 'unknown')}")
            await self.broadcast({
                'type': 'system',
    ...
    async def _continuous_trading_loop(self):
        ...
        while True:
            ...
            # Check if it's a trading day (using US date)
            us_date = (now_beijing - timedelta(hours=12)).strftime("%Y-%m-%d")  # Rough conversion to US date
            is_trading_day = self._is_trading_day(us_date)
            
            if not is_trading_day:
                # Non-trading day: only maintain page updates
                await self._handle_non_trading_day(now_beijing)
                await self.vclock.sleep(60)  # Check every minute (virtual time)
                continue
            ...
            if should_run_analysis:
                print(f"🔍 Detected pre-market analysis time window | analysis_executed_today={self.analysis_executed_today} | us_date={us_date}")
            if should_execute_trade:
                print(f"🔍 Detected trade execution time window | trading_executed_today={self.trading_executed_today} | us_date={us_date}")
            ...
            if should_run_analysis and not self.analysis_executed_today:
                ...
                print(f"🎯 Triggering pre-market analysis (func1) | us_date={us_date} | Beijing time={now_beijing.strftime('%H:%M:%S')}")
                ...
                print(f"🎯 Triggering trade execution (func2) | us_date={us_date} | Beijing time={now_beijing.strftime('%H:%M:%S')}")
                await self._run_trade_execution_with_prev_update(us_date)
                self.trading_executed_today = True
                self.last_trading_date = us_date
                self.last_executed_date = us_date  # Record actual execution date
                await self.vclock.sleep(300)  # Wait 5 minutes after execution (virtual time)
            ...
            if datetime_time(20, 0) <= current_time < datetime_time(22, 29):
                ...
                if self.last_executed_date and us_date != self.last_executed_date:
                    if self.trading_executed_today or self.analysis_executed_today:
                        logger.info(f"📅 Detected trading day change ({self.last_executed_date} → {us_date}), resetting daily flags")
                        logger.info(f"   Beijing time={now_beijing.strftime('%H:%M:%S')}")
                        logger.info(f"   Before reset: trading_executed={self.trading_executed_today}, analysis_executed={self.analysis_executed_today}")
                        self.trading_executed_today = False
                        self.analysis_executed_today = False
                        logger.info(f"   After reset: trading_executed={self.trading_executed_today}, analysis_executed={self.analysis_executed_today}")
    ...
    async def _handle_non_trading_day(self, now_beijing: datetime):
        ...
        if current_phase != 'non_trading_day':
            ...
            logger.info(f"📅 Non-trading day: {now_beijing.strftime('%Y-%m-%d')}")
        
        # Broadcast time update
        next_open = self._get_next_market_open_time_beijing()
        hours_to_open = (next_open - now_beijing).total_seconds() / 3600
        
        logger.info(f"⏰ Current time: {now_beijing.strftime('%Y-%m-%d %H:%M:%S')} | Status: Non-trading day | Hours to open: {hours_to_open:.1f}")
    ...
    async def _handle_market_open_period(self, now_beijing: datetime, trading_date: str):
        ...
        if current_phase != 'market_open':
            ...
            logger.info(f"📊 Market open period: {now_beijing.strftime('%H:%M:%S')}")
        ...
        logger.info(f"⏰ Current time: {now_beijing.strftime('%Y-%m-%d %H:%M:%S')} | Status: Market open | Hours to trade execution: {hours_to_trade:.1f}")
    ...
    async def _handle_off_market_period(self, now_beijing: datetime):
        ...
        if current_phase not in ['off_market', 'trade_execution']:
            ...
            logger.info(f"⏸️ Off-market period: {now_beijing.strftime('%H:%M:%S')}")
        
        # Broadcast time update
        next_open = self._get_next_market_open_time_beijing()
        hours_to_open = (next_open - now_beijing).total_seconds() / 3600
        
        logger.info(f"⏰ Current time: {now_beijing.strftime('%Y-%m-%d %H:%M:%S')} | Status: Off-market period | Hours to open: {hours_to_open:.1f}")
    ...
    async def _run_data_updater(self):
        ...
        if process.returncode == 0:
            ...
            logger.warning(f"⚠️ [Scheduled Task] Historical data update failed: {error_msg[:200]}")
            await self.broadcast({
                'type': 'system',
    ...
    async def _daily_data_updater_scheduler(self):
        ...
        logger.info("📅 Data update scheduler started (executes daily at 05:10)")
        ...
        try:
            while True:
                ...
                logger.info(f"⏰ Next data update time: {next_run.strftime('%Y-%m-%d %H:%M:%S')} (waiting {wait_seconds/3600:.2f} hours)")
...
async def main():
    ...
    logger.info(f"   Run mode: {'🎭 MOCK (virtual prices)' if args.mock else '🚀 LIVE (real-time prices)'}")
    ...
    if pause_before_trade:
        logger.info(f"   Trade execution: ⏸️ Pause mode (analysis only, no trade execution) [Source: {pause_source}]")
    else:
        logger.info(f"   Trade execution: ▶️ Normal mode (execute trades after analysis)")
    if args.time_accelerator != 1.0:
        logger.info(f"   ⚡ Time acceleration: {args.time_accelerator}x (debug mode)")
    ...
    if args.virtual_start_time and args.mock:
        ...
        virtual_start_time = virtual_start_time.replace(tzinfo=timezone(timedelta(hours=8)))  # Beijing time
        logger.info(f"   🕐 Virtual start time: {virtual_start_time.strftime('%Y-%m-%d %H:%M:%S')} (Beijing time)")
