...
class DataUpdater:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error(
                "❌ finnhub-python package not installed, please run: pip install finnhub-python",
            )
            raise
    ...
    def fetch_data_from_api(
        ...
        try:
            ...
            logger.info(
                f"🔄 Fetching {ticker} data: {start_date.date()} to {end_date.date()}",
            )
            ...
            if data.get("s") != "ok":
                logger.warning(
                    f"⚠️ {ticker} API returned abnormal status: {data.get('s')}",
                )
                return None
            ...
            logger.info(
                f"✅ Successfully fetched {ticker} data: {len(df)} records",
            )
            return df
    ...
    def merge_and_save(
        ...
        try:
            if csv_path.exists():
                ...
                logger.info(
                    f"📊 {ticker} existing data: {len(old_data)} records",
                )
    ...
    def _fill_missing_dates(
        ...
        try:
            ...
            all_trading_dates_str = self.get_trading_dates(
                start_date,
                end_date,
            )
            ...
            missing_dates = [
                d for d in all_trading_dates if d.date() not in existing_dates
            ]
            ...
            if missing_dates:
                ...
                logger.info(
                    f"✅ {ticker} filled {len(missing_dates)} missing dates",
                )
    ...
    def update_ticker(
        ...
        if force_full_update:
            ...
            logger.info(
                f"🔄 Force full update, start date: {start_date.date()}",
            )
        else:
...
def main():
    ...
    if not api_key:
        ...
        logger.error(
            "   Please set via --api-key parameter or FINNHUB_API_KEY environment variable",
        )
        sys.exit(1)
    ...
    if success_count == len(results):
        ...
        logger.warning(
            "⚠️ All stocks have no new data (may be weekend/holiday), will use existing data",
        )
        ...
        logger.warning(
            "⚠️ Some stocks failed to update, but will continue running",
        )
        sys.exit(0)  # Return success to let server continue starting
