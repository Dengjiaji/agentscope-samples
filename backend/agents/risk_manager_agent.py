...
class RiskManagerAgent(AgentBase):
    ...
    def _load_or_default_position_config(self) -> Dict[str, Any]:
        ...
        try:
            return self.prompt_loader.load_yaml_config(
                self.agent_type,
                "position_limits",
            )
        ...
        for ticker in tickers:
            ...
            if not historical_prices:
                progress.update_status(
                    self.agent_id,
                    ticker,
                    "Warning: Historical price data not found",
                )
    ...
    def _generate_basic_risk_analysis(
        self,
        tickers: list[str],
    ...
    def _generate_portfolio_risk_analysis(
        self,
        tickers: list[str],
        volatility_data: Dict[str, Dict],
        current_prices: Dict[str, float],
        ...
        for ticker, position in portfolio.get("positions", {}).items():
            if ticker in current_prices:
                ...
                total_value -= (
                    position.get("short", 0) * current_prices[ticker]
                )
        ...
        for ticker in tickers:
            ...
            annualized_vol = vol_data.get("annualized_volatility", 0.25)
            vol_adjusted_limit = self._calculate_volatility_adjusted_limit(
                annualized_vol,
            )
    ...
    def _calculate_volatility_metrics(
        ...
        recent_returns = daily_returns.tail(
            min(lookback_days, len(daily_returns)),
        )
    ...
    def _calculate_volatility_adjusted_limit(
        self,
