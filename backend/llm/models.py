...
class ModelWrapper:
    ...
    def _init_openai_model(self):
        ...
        api_key = self.api_keys.get("OPENAI_API_KEY") or os.getenv(
            "OPENAI_API_KEY",
        )
    ...
    def _init_anthropic_model(self):
        ...
        api_key = self.api_keys.get("ANTHROPIC_API_KEY") or os.getenv(
            "ANTHROPIC_API_KEY",
        )
        if not api_key:
    ...
    def _init_groq_model(self):
        ...
        api_key = self.api_keys.get("GROQ_API_KEY") or os.getenv(
            "GROQ_API_KEY",
        )
        if not api_key:
    ...
    def _init_deepseek_model(self):
        ...
        api_key = self.api_keys.get("DEEPSEEK_API_KEY") or os.getenv(
            "DEEPSEEK_API_KEY",
        )
        if not api_key:
    ...
    def _init_google_model(self):
        ...
        api_key = self.api_keys.get("GOOGLE_API_KEY") or os.getenv(
            "GOOGLE_API_KEY",
        )
        if not api_key:
    ...
    def _init_openrouter_model(self):
        ...
        api_key = self.api_keys.get("OPENROUTER_API_KEY") or os.getenv(
            "OPENROUTER_API_KEY",
        )
        if not api_key:
    ...
    def _init_gigachat_model(self):
        ...
        api_key = self.api_keys.get("GIGACHAT_API_KEY") or os.getenv(
            "GIGACHAT_API_KEY",
        )
        if not api_key:
