...
def normalize_pandas(data):
    ...
    if isinstance(data, dict):
        ...
        return {
            key: safe_float(value) for key, value in data.to_dict().items()
        }
    else:
...
def analyze_trend_following(
    ...
    try:
        ...
        if len(prices_df) < 10:
            ...
            print(
                f"Warning: Insufficient data for trend analysis, only {len(prices_df)} days available, recommend at least 20 days",
            )
        ...
        sma_long_window = (
            min(50, data_length - 5)
            if data_length > 25
            else min(25, data_length - 5)
        )
        sma_200_window = (
            min(200, data_length - 10) if data_length > 200 else None
        )

        prices_df["SMA_20"] = (
            prices_df["close"].rolling(window=sma_short_window).mean()
        )
        prices_df["SMA_50"] = (
            prices_df["close"].rolling(window=sma_long_window).mean()
        )
        if sma_200_window:
            prices_df["SMA_200"] = (
                prices_df["close"].rolling(window=sma_200_window).mean()
            )
        prices_df["EMA_12"] = (
            prices_df["close"].ewm(span=min(12, data_length // 3)).mean()
        )
        prices_df["EMA_26"] = (
            prices_df["close"].ewm(span=min(26, data_length // 2)).mean()
        )

        # MACD
        prices_df["MACD"] = prices_df["EMA_12"] - prices_df["EMA_26"]
        prices_df["MACD_signal"] = prices_df["MACD"].ewm(span=9).mean()
        prices_df["MACD_histogram"] = (
            prices_df["MACD"] - prices_df["MACD_signal"]
        )
        ...
        sma_200 = (
            safe_float(prices_df["SMA_200"].iloc[-1])
        ...
        if sma_200 is not None:
            long_term_trend = (
                "bullish" if current_price > sma_200 else "bearish"
            )
...
def analyze_mean_reversion(
    ...
    try:
        ...
        if len(prices_df) < 5:
            ...
            print(
                f"Warning: Insufficient data for mean reversion analysis, only {len(prices_df)} days available, recommend at least 20 days",
            )
...
def analyze_momentum(
    ...
    try:
        ...
        if len(prices_df) < 5:
            ...
            print(
                f"Warning: Insufficient data for momentum analysis, only {len(prices_df)} days available, recommend at least 20 days",
            )
        ...
        prices_df["momentum_5"] = (
            prices_df["close"] / prices_df["close"].shift(short_period) - 1
        )
        prices_df["momentum_10"] = (
            prices_df["close"] / prices_df["close"].shift(medium_period) - 1
        )
        prices_df["momentum_20"] = (
            prices_df["close"] / prices_df["close"].shift(long_period) - 1
        )
        ...
        recent_volatility = safe_float(
            prices_df["returns"].tail(20).std() * np.sqrt(252),
        )
...
def analyze_volatility(
    ...
    try:
        ...
        if len(prices_df) < 5:
            ...
            print(
                f"Warning: Insufficient data for volatility analysis, only {len(prices_df)} days available, recommend at least 20 days",
            )
        ...
        long_window = (
            min(60, data_length - 1) if data_length > 30 else medium_window
        )

        prices_df["vol_10"] = prices_df["returns"].rolling(
            window=short_window,
        ).std() * np.sqrt(252)
        prices_df["vol_20"] = prices_df["returns"].rolling(
            window=medium_window,
        ).std() * np.sqrt(252)
        prices_df["vol_60"] = prices_df["returns"].rolling(
            window=long_window,
        ).std() * np.sqrt(252)
...
def analyze_insider_trading(
    ...
    try:
        ...
        total_sell_volume = abs(
            transaction_shares[transaction_shares < 0].sum(),
        )
        ...
        return {
            "metrics": {
                ...
                "volume_ratio": round(
                    total_buy_volume / max(total_sell_volume, 1),
                    2,
                ),
            },
...
def analyze_news_sentiment(
    ...
    try:
        ...
        sentiment_data = pd.Series(
            [n.sentiment for n in company_news],
        ).dropna()
...
def ev_ebitda_valuation_analysis(
    ...
    try:
        ...
        current_ebitda = (
            most_recent.enterprise_value
            / most_recent.enterprise_value_to_ebitda_ratio
        )
        ...
        # Value gap
        value_gap = (
            (implied_equity_value - market_cap) / market_cap
            if market_cap > 0
            else 0
        )
        ...
        # Multiple comparison
        multiple_discount = (
            median_multiple - current_multiple
        ) / median_multiple
...
def residual_income_valuation_analysis(
    ...
    try:
        ...
        for year in range(1, num_years + 1):
            future_ri = (
                initial_residual_income * (1 + book_value_growth) ** year
            )
            pv_residual_income += future_ri / (1 + cost_of_equity) ** year

        # Terminal value
        terminal_ri = initial_residual_income * (1 + book_value_growth) ** (
            num_years + 1
        )
