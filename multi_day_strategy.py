...
class MultiDayStrategy:
    ...
    def _initialize_portfolio_state(self):
        ...
        if latest_state:
            ...
            positions_count = len([p for p in latest_state.get('positions', {}).values() 
                                  if p.get('long', 0) > 0 or p.get('short', 0) > 0])
            print(f"✅ Loaded portfolio state from disk: Cash ${latest_state['cash']:,.2f}, "
            ...
            print(f"✅ Initialized portfolio state: Cash ${self.initial_cash:,.2f}")
    ...
    def _load_latest_portfolio_state(self) -> Optional[Dict[str, Any]]:
        ...
        if portfolio_files:
            ...
            try:
                ...
                print(f"⚠️ Failed to load portfolio state ({latest_file}): {e}")
        return None
    ...
    def run_single_day(
        ...
        if self.mode == "portfolio" and self.portfolio_state:
            positions_count = len([p for p in self.portfolio_state.get('positions', {}).values() 
                                  if p.get('long', 0) > 0 or p.get('short', 0) > 0])
            print(f"📌 Using existing portfolio state: Cash ${self.portfolio_state['cash']:,.2f}, "
                  f"Positions {positions_count}")
        ...
        if self.mode == "portfolio" and "updated_portfolio" in result:
            ...
            if updated_portfolio:
                ...
                if updated_portfolio.get('margin_used', 0) > 0:
                    print(f"   Margin used: ${updated_portfolio['margin_used']:,.2f}")
    ...
    def run_multi_day(
        ...
        if self.session_id is None:
            self.session_id = f"multi_day_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        print(f"Starting multi-day strategy analysis (Session ID: {self.session_id})")
    ...
    def _calculate_stock_daily_return_from_signal(self, ticker: str, date, action: str) -> tuple:
        """
        ...
        if action == "long":
            ...
            return float(-market_return), float(market_return), float(price)  # Short: get opposite of market return
        else:  # hold
